...
const logger = require('../src/config/logger'), { responderErroInterno } = require('../src/utils/responderErroInterno');
function lerJavaScript(dir) {
  return fs.readdirSync(dir, { withFileTypes: true }).flatMap((entry) => { const arquivo = path.join(dir, entry.name); return entry.isDirectory() ? lerJavaScript(arquivo) : entry.name.endsWith('.js') ? [fs.readFileSync(arquivo, 'utf8')] : []; });
}
...
try{const a=require(${JSON.stringify(app)}),h=require('http'),s=h.createServer(a).listen(0,'127.0.0.1',()=>h.get({port:s.address().port,path:'/ready'},r=>{let b='';r.on('data',x=>b+=x);r.on('end',()=>{process.stdout.write('booted '+b);s.close(()=>process.exit(0))})}))}catch(e){process.stderr.write(e.message);process.exitCode=1}`;
    const run = (route) => spawnSync(process.execPath, ['-e', script], { cwd: path.join(__dirname, '..'), encoding: 'utf8', env: { ...process.env, FAIL_ROUTE: route, SAGE_DATA_DIR: data, NODE_ENV: 'test' } });
