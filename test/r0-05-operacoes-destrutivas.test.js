...
describe('R0-05 — backup e destruição', () => {
  ...
  it('A-004: gates e falha local não confirmam nem preservam deletes parciais', async () => {
    ...
    await comFakes([
      ...
      backup.verificarBackup = async () => ({ ok: false }); res = resposta(); await controller.comecarDoZero({ params: { id: '1' }, body: { confirmacao: 'APAGAR TUDO' }, user: { id: 7 } }, res); expect(zeragens).toBe(0);
      backup.verificarBackup = async () => ({ ok: true }); res = resposta(); await controller.comecarDoZero({ params: { id: '1' }, body: { confirmacao: 'APAGAR TUDO', apagarAcessosNoSistema: true }, user: { id: 7 } }, res); expect({ commits, rollbacks, restores }).toEqual({ commits: 0, rollbacks: 1, restores: 1 }); expect(res.status).toHaveBeenCalledWith(500);
      device.zerarTudoNaCatraca = async () => ({ ok: false }); res = resposta(); await controller.comecarDoZero({ params: { id: '1' }, body: { confirmacao: 'APAGAR TUDO' }, user: { id: 7 } }, res); expect(restores).toBe(2);
