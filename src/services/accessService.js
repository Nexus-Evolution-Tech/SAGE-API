...
async function sincronizarAcessos(dispositivo) {
  ...
  if (!session) {
    return {
      sucesso: false,
      message: `Erro ao obter sessão com a catraca ${dispositivo.nome}`,
    };
  }

  // Busca o último acesso sincronizado para este dispositivo
  const ultimoAcesso = await global
    .db("Acesso")
  ...
  const logs = await deviceService.obterLogsCatraca(
    session,
    link,
    timestampInicial
  );
  ...
  for (const log of logs) {
    ...
    const pessoa_id = parseInt(log.user_id);


    const dispositivo_id = dispositivo.id;
    const data_hora = new Date((log.time + 3 * 60 * 60) * 1000);
    const status = identificarAcesso(log.portal_id);
    const metodo_auth = mapearMetodo(log.card_value);
    const permitido = true;

    const pessoaExiste = await global
      .db("Pessoa")
      .where("id", pessoa_id)
      .first();

    if (pessoaExiste) {
      // Verifica se já existe um registro com esses dados (Evita duplicidade)
      const acessoExistente = await global
        .db("Acesso")
      ...
      console.warn(
        `Aviso: Log encontrado para user_id ${pessoa_id}, mas usuário não existe no banco de dados.`
      );
    }
  }
...
async function sincronizarTodosAcessos() {
  // Busca todas as catracas no banco
  const dispositivos = await global.db("Dispositivo");
  const resultados = [];

  // Percorre CADA dispositivo encontrado (loop)
  for (const dispositivo of dispositivos) {
    try {
      console.log(`Iniciando sincronização da catraca: ${dispositivo.nome}`);
      const resultado = await sincronizarAcessos(dispositivo);
      resultados.push(resultado);
    } catch (error) {
      // O try/catch é importante para que, se uma catraca falhar (ex: sem rede),
      // a outra continue funcionando.
      console.error(`Erro ao sincronizar ${dispositivo.nome}:`, error);
      resultados.push({
        sucesso: false,
        dispositivo_id: dispositivo.id,
        nome: dispositivo.nome,
...
async function criarAcesso(dados) {
  ...
  if (status === "ENTRADA") {
    if (ultimoAcesso && ultimoAcesso.status === "ENTRADA") {
      return {
        message:
          "Acesso negado: Usuário já está dentro, não pode entrar novamente sem sair",
      };
    }
    ...
    if (!ultimoAcesso || ultimoAcesso.status !== "ENTRADA") {
      return {
        message:
          "Acesso negado: Usuário não está dentro, não pode sair sem entrar antes",
      };
    }
  }
  ...
  switch (status) {
    ...
    case "SAIDA":
      ...
      const solicitacoes = await global
        .db("SolicitacaoAcesso")
      ...
      const solicitacao = solicitacoes.find((s) =>
        ["APROVADA", "NEGADA", "PENDENTE"].includes(s.status)
      );
      ...
      if (!solicitacao) {
        ...
        return {
          message:
            "Acesso negado: Solicitação de saída criada e aguardando aprovação da secretaria",
        ...
        switch (solicitacao.status) {
          case "APROVADA":
            ...
            if (mesmaData) {
              ...
              return {
                message:
                  "Acesso negado: Solicitação aprovada, mas tempo de validez expirado",
                idade_aluno: `Aluno com ${idadePessoa} anos - MENOR DE IDADE`,
                hora_atual: agora,
              };
            }
          case "NEGADA":
            ...
            return {
              message:
                "Acesso negado: Solicitação de saída foi negada pela secretaria",
              idade_aluno: `Aluno com ${idadePessoa} anos - MENOR DE IDADE`,
              hora_atual: agora,
            };
          case "PENDENTE":
            ...
            return {
              message:
                "Acesso negado: Solicitação ainda não foi aprovada pela secretaria",
              idade_aluno: `Aluno com ${idadePessoa} anos - MENOR DE IDADE`,
              hora_atual: agora,
            };
          default:
            ...
            return {
              message:
                "Acesso negado: Solicitação de saída com status desconhecido",
              idade_aluno: `Aluno com ${idadePessoa} anos - MENOR DE IDADE`,
              hora_atual: agora,
            };
        }
