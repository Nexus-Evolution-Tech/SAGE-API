...
const USER_ID_OFFSET = parseInt(
  process.env.CATRACA_USER_ID_OFFSET || "110000000",
);
...
async function sincronizarAcessos(dispositivo) {
  ...
  for (const log of logs) {
    ...
    const [pessoaResult] = await db.query(
      "SELECT * FROM Pessoa WHERE id = ? LIMIT 1",
      [userId],
    );
    ...
    //PARA PASSAR O ID DAS PESSOAS É PRECISO DELETAR TODOS OS OUTROS DADOS ANTERIORES
...
async function criarAcesso(dados) {
  ...
  const pessoa = pessoaResult[0];
  if (!pessoa)
    return { message: "Pessoa não encontrada", error: "PESSOA_INEXISTENTE" };
  ...
  if (status === "ENTRADA" && ultimoAcesso?.status === "ENTRADA") {
    return {
      message: "Usuário já está dentro, não pode entrar novamente sem sair",
    };
  }
  if (
    status === "SAIDA" &&
    (!ultimoAcesso || ultimoAcesso.status !== "ENTRADA")
  ) {
    return {
      message: "Usuário não está dentro, não pode sair sem entrar antes",
    };
  }
  ...
  const [acessoResult] = await db.query(
    "SELECT * FROM Acesso WHERE id = LAST_INSERT_ID() LIMIT 1",
  );
