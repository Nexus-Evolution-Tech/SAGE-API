...
const obterUnidadeAtual = async (req, res) => {
  try {
    ...
    const unidade = await buscarUnidadeSingleton(camposSemSenha);
    res.json(unidade);
  } catch (error) {
    if (error.code === ERRO_UNIDADE_SINGLETON) {
      logger.error(`[UNIDADE] codigo=${ERRO_UNIDADE_SINGLETON}`);
      return res.status(503).json({ message: 'Unidade escolar indisponível', code: ERRO_UNIDADE_SINGLETON });
    }
    logger.error('[UNIDADE] codigo=CONSULTA_UNIDADE_FALHOU');
    res.status(500).json({ message: 'Erro ao carregar dados da unidade', error: 'CONSULTA_UNIDADE_FALHOU' });
  }
};
...
const atualizarUnidadeAtual = async (req, res) => {
  try {
    ...
    const unidade = await buscarUnidadeSingleton();
    await crud.atualizarRegistro(tabela, unidade.id, updates);
    ...
    logger.error('[UNIDADE] codigo=ATUALIZACAO_UNIDADE_FALHOU');
    res.status(500).json({ message: 'Erro ao atualizar dados', error: 'ATUALIZACAO_UNIDADE_FALHOU' });
  }
};
  ...
  try {
    ...
    if (arquivoTemporario && fs.existsSync(arquivoTemporario)) {
      try {
        ...
        logger.warn('[UNIDADE-LOGO] codigo=ARQUIVO_TEMPORARIO_NAO_REMOVIDO');
      }
    }
    if (error.code === ERRO_UNIDADE_SINGLETON) {
      logger.error(`[UNIDADE] codigo=${ERRO_UNIDADE_SINGLETON}`);
    ...
    logger.error('[UNIDADE-LOGO] codigo=UPLOAD_LOGO_FALHOU');
    res.status(500).json({ message: 'Erro ao salvar a logo' });
  }
};
